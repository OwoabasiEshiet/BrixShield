...
<head>
    ...
    <title>BrixShield Security Report</title>
    <style>
        body {
...
<body>
    <div class="header">
        <h1>🛡️ BrixShield Security Report</h1>
    ...
    <div class="footer">
        <p>This report was generated by BrixShield Security Scanner</p>
...
export function downloadSingleScanReport(scan: ScanResult, format: 'json' | 'html' = 'json') {
  ...
  if (format === 'json') {
    downloadJSON(reportData, `brixshield-${scanType}-scan-${sanitizedTarget}-${timestamp}.json`)
  } else {
    downloadHTML(reportData, `brixshield-${scanType}-scan-${sanitizedTarget}-${timestamp}.html`)
  }
}